...
- **Auto Mode awareness (autopilot).** When the runtime is in Auto Mode (Copilot CLI's `continueOnAutoMode`, changelog 76), the runtime may transparently switch models on rate-limit. CRISPY's `mode: autopilot` is layered above this — it does not require Auto Mode to function, but autopilot checkpoint summaries should mention an Auto-Mode model switch if one occurred during the phase. Once per session, after the first phase completes in autopilot, append a one-line reminder to the checkpoint: *"Tip: enable Auto Mode (`continueOnAutoMode`, changelog 76) for resilient long-running autopilot runs."* Track emission via `review-gates.yaml` field `AutoModeReminderEmitted: true` keyed on `<session_id>` to debounce.
- **Background-task UX.** Long-running phase sub-agents (especially `crispy-research` background spawns and any `crispy-implement` fleet wave) may take more than 30s. Users can press `ctrl+x → b` (changelog 5) to send a running task to the background and continue interacting with the orchestrator; checkpoint summaries should mention this when a phase's wall time exceeds the 30s threshold.
